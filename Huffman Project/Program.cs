...
namespace Huffman_Project
{
    public class HuffmanNode : IComparable<HuffmanNode>
    {
        ...
        public HuffmanNode(char? character, ulong occ)
        {
            Left = Right = null;
    ...
    {
        static void ExtractCodes(HuffmanNode root, string binary, Dictionary<char, string> codesDict)
        {
            if (root == null)
                return;
        ...
        {
            var minHeap = new MinHeap<HuffmanNode>(frequencies.Count);

            foreach (var freq in frequencies)
                minHeap.Add(new HuffmanNode(freq.Key, freq.Value));
            ...
            while (minHeap.Count != 1)
            {
                ...
                var top = new HuffmanNode(null, left.Occurences + right.Occurences);
